...
def save_one_json_with_gt(predn, jdict, path, class_map, labelsn):
    # Save one JSON result {"image_id": 42, "category_id": 18, "bbox": [258.15, 41.29, 348.26, 243.78], "score": 0.236}
    if len(predn) > 0 and len(labelsn) > 0:
        image_id = int(path.stem) if path.stem.isnumeric() else path.stem
        box = predn[:, :4] #xyxy2xywh(predn[:, :4])  # xywh
        #box[:, :2] -= box[:, 2:] / 2  # xy center to top-left corner
        jdict[image_id] = {"detections":[], "labels":[]}
        for p, b in zip(predn.tolist(), box.tolist()):
            jdict[image_id]["detections"].append({'bbox':[round(x, 3) for x in b], 'score':round(p[4], 5), 'category_id':class_map[int(p[5])]})
        box = labelsn[:, 1:] #xyxy2xywh(labeln[:, 1:])
        #box[:, :2] -= box[:, 2:] / 2  # xy center to top-left corner 
        for p, b in zip(labelsn[:, 0], box.tolist()):
            jdict[image_id]["labels"].append({'bbox':[round(x, 3) for x in b], 'category_id':int(p)})
...
def run(data,
    ...
    jdict = [] if save_json else {}
    stats, ap, ap_class = [], [], []

    warmup_epochs = list(range(20))
    ...
    for batch_i, (img, targets, paths, shapes, main_target_indices, label_paths) in enumerate(tqdm(dataloader, desc=s)):
        ...
        if batch_i not in warmup_epochs:
            t2 = time_sync()
            dt[0] += t2 - t1
        ...
        for si, pred in enumerate(out):
            ...
            if save_json_gt:
                save_one_json_with_gt(predn, jdict, path, class_map, labelsn)  # append to COCO-JSON dictionary
    ...
    # Save JSON
    if (save_json or save_json_gt) and len(jdict):
